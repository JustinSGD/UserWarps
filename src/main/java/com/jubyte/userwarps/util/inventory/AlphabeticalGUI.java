...
public class AlphabeticalGUI extends PaginatedMenu {

    private final List<LocationEntry> warpList = new ArrayList<>(UserwarpLocationSQL.getWarps());

    public String getTitle() {
        return ConfigData.ALPHABETICAL_GUI_TITLE;
    }
    ...
    public void buildGUI() {
        ...
        fill();
        warpList.sort((o1, o2) -> {
            if (o1.getWarpName() == null && o2.getWarpName() == null) {
                return 0;
            }
            if (o1.getWarpName() == null) {
                return -1;
            }
            return o1.getWarpName().toLowerCase().compareTo(o2.getWarpName().toLowerCase());
        });
        addContent(warpList);
        setButtons(warpList);
    }

    public void addContent(List<LocationEntry> playerWarps) {
        String warpItem = ConfigData.ALPHABETICAL_GUI_ITEMS_WARP;
        for (int i = 0; i < getMaxItemsPerPage(); i++) {
            this.index = getMaxItemsPerPage() * this.page + i;
            if (this.index >= playerWarps.size()) break;
            if (playerWarps.get(this.index) != null) {
                UUID uuid = UUID.fromString(UserwarpPlayerSQL.getPlayerById(playerWarps.get(this.index).getPlayerID()));
                OfflinePlayer offlinePlayer = Bukkit.getServer().getOfflinePlayer(uuid);
                this.inventory.addItem(new ItemBuilder(Material.BOOK).setDisplayName(warpItem.replace("[warpName]", playerWarps.get(this.index).getWarpName()))
